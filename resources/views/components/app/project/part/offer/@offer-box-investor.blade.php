        ...
        <div>
            <div class="font-Spartan-Bold text-[13px] leading-[22px] text-[#414141]">
                @if($project->type === 'preliminary-interest')
                    {{ __('Pro projevení předběžného zájmu a zobrazení všech údajů se musíte přihlásit jako investor a mít ověřený účet.') }}
                @else
                    {{ __('Pro zaslání nabídky a zobrazení všech údajů se musíte přihlásit jako investor a mít ověřený účet.') }}
                @endif
            </div>
...
@elseif(!auth()->user()->isVerified())
    <div class="grid gap-x-[20px] mb-[25px] grid-cols-1 laptop:grid-cols-2">
        <div class="font-Spartan-Bold text-[13px] leading-[22px] text-[#414141]">
            @if($project->type === 'preliminary-interest')
                {{ __('Pro zobrazení všech údajů nebo projevení předběžného zájmu musíte mít ověřený účet investora.') }}
            @else
                {{ __('Pro zobrazení všech údajů nebo zaslání nabídky musíte mít ověřený účet investora.') }}
            @endif
        </div>
        <div class="text-center laptop:text-right">
...
@elseif(auth()->user()->investor)
    ...
    @elseif(!$project->isPublicForInvestor())
        <div class="grid gap-x-[20px] mb-[25px] grid-cols-1 laptop:grid-cols-[1fr_200px]">
            <div class="font-Spartan-Bold text-[13px] leading-[22px] text-[#414141]">
                {{ __('U tohoto projektu vyžaduje nabízející vyšší stupeň ověření investorů. O zobrazení detailů o projektu musíte nabízejícího požádat.') }}
            </div>
            ...
            @elseif($project->myShow()->first()->details_on_request === -1)
                <div class="font-Spartan-SemiBold text-[18px] leading-[24px] text-app-red">
                    {{ __('Nabízející vám neschválil plný přístup k projektu. Nabídku nemůžete učinit.') }}
                </div>
            @endif
        </div>
    @elseif($project->type === 'auction' && $project->myShow()->first()->principal_paid !== 1)
        <div class="grid gap-x-[20px] mb-[25px] grid-cols-1 laptop:grid-cols-2">
            <div class="font-Spartan-Bold text-[13px] leading-[22px] text-[#414141]">
                {{ __('Abyste mohli učinit podání, musíte uhradit jistotu na bankovní účet provozovatele.') }}
            </div>
            <div class="text-center laptop:text-right">
                <button type="button"
                        class="text-center inline-block self-center font-Spartan-SemiBold bg-app-green text-white text-[18px] h-[60px] leading-[60px] w-[200px] rounded-[3px] shadow-[0_3px_6px_rgba(0,0,0,0.16)]
                ...
                <div class="grid grid-cols-1" x-data="{offerPrice: '',
                    ...
                    @if($project->type === 'preliminary-interest')
                        ...
                        <div class="font-Spartan-Bold text-[13px] leading-[29px] text-[#414141] mb-[24px]">
                            {{ __('Projev zájmu je nezávazný. Vlastníkovi projektu budete identifikováni, domluvíte se na dalším postupu, nebo budete zavčas informováni, pokud ho dá k prodeji na naši platformu.') }}
                        </div>
                    @endif
                            ...
                            async addOffer(offer) {
                                ...
                                await fetch('/projekty/add-offer', {
                                    ...
                                    .then((data) => {
                                        ...
                                        alert(@js(__('Nepodařilo se vložit nabídku')))
                                        this.loaderShow = false;
                                    })
                                    .catch((error) => {
                                        alert(@js(__('Chyba vložení nabídky')))
                                        this.loaderShow = false;
                                    });
                            }
                            }">
                                ...
                                <div class="text-center mb-[30px]">
                                    @if($project->type === 'preliminary-interest')
                                        <h1>{{ __('Podání předběžného zájmu') }}</h1>
                                    @elseif($project->type === 'auction')
                                        <h1>{{ __('Potvrzení podání') }}</h1>
                                    @else
                                        <h1>{{ __('Podání nabídky') }}</h1>
                                    @endif
                                </div>
                                ...
                                @if($project->type === 'preliminary-interest')
                                    ...
                                    after:absolute after:bg-[url('/resources/images/ico-info-orange.svg')] after:w-[20px] after:h-[20px] after:left-[15px] after:top-[15px]">
                                            <div class="text-left">
                                                <p class="mb-[10px]">
                                                    <span
                                                        class="font-Spartan-SemiBold">{{ __('Podáním předběžného zájmu se nezavazujete ke koupi projektu.') }}</span>
                                                    <br><br>
                                                    {{ __('To, že zájem projevíte, má však důležité dopady na vaši potenciální účast v projektu.') }}
                                                    <span class="font-Spartan-SemiBold">
                                                        {{ __('Budoucímu prodávajícímu budete jako možný investor identifikováni.') }}
                                                    </span>
                                                    {{ __('Mohou tak, bude-li o to mít zájem, započít případná přímá jednání o dalším postupu.') }}
                                                    <br><br>
                                                    {{ __('Následně mohou nastat tyto scénáře:') }}<br>
                                                    {{ __('1. Domluvíte se na tom, že do projektu vstoupíte jako spoluinvestor, nebo se na něm budete jakkoliv jinak podílet.') }}
                                                    <br>
                                                    {{ __('2. Domluvíte se na budoucím prodeji.') }}<br>
                                                    {{ __('3. Prodávající projekt, jakmile bude způsobilý k prodeji, umístí na naši platformu. A vy budete na základě projeveného předběžného zájmu informováni a budete moct učinit nabídku.') }}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                @elseif($project->type === 'auction')
                                    ...
                                    after:absolute after:bg-[url('/resources/images/ico-info-orange.svg')] after:w-[20px] after:h-[20px] after:left-[15px] after:top-[15px]">
                                            <div class="text-left">
                                                <p class="mb-[10px]">
                                                    <span class="font-Spartan-SemiBold">
                                                        {{ __('Potvrzením podání závazně projevujete zájem o koupi projektu.') }}
                                                    </span>
                                                    {{ __('V případě, že od záměru ustoupíte z důvodů na své straně, dojde k propadnutí vámi zaplacené jistoty ve prospěch provozovatele.') }}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                @else
                                    ...
                                    after:absolute after:bg-[url('/resources/images/ico-info-orange.svg')] after:w-[20px] after:h-[20px] after:left-[15px] after:top-[15px]">
                                            <div class="text-left">
                                                <p class="mb-[10px]">
                                                    <span class="font-Spartan-SemiBold">
                                                        {{ __('Podáním nabídky se určuje pořadí projeveného zájmu o koupi.') }}
                                                    </span>
                                                    {{ __('Aby vaše nabídka byla platná, je třeba uhradit') }}
                                                    <span class="font-Spartan-SemiBold">{{ __('jistotu') }}</span>,
                                                    {{ __('jejíž výše je uvedena u projektu.') }}
                                                </p>
                                                <p class="mb-[10px]">
                                                    {{ __('Po potvrzení vašeho zájmu podáním nabídky vás budeme kontaktovat a zašleme vám instrukce k úhradě jistoty.') }}
                                                </p>
                                                <p>
                                                    {{ __('Jistota musí být připsána na náš účet nejpozději do dvou pracovních dní od zaslání instrukcí k úhradě. V opačném případě může být vaše nabídka z pořadníku vyloučena.') }}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
...
@else
    <div class="grid gap-x-[20px] mb-[25px]
        ...
        <div>
            <div class="font-Spartan-Bold text-[13px] leading-[22px] text-[#414141]">
                {{ __('Abyste mohli vidět všechny údaje o projektu, musíte v “Nastavení účtu” přidat typ účtu “Účet investora” a projít procesem ověření.') }}
            </div>
        </div>
        <div class="text-center laptop:text-right">

        ...
        <section class="max-w-7xl mx-auto py-4 px-5" x-data="{exclusive: @js($project->exclusive_contract ? 1 : 0)}">
            ...
            <form method="post" action="{{ route('admin.projects.save', ['project' => $project]) }}"
                  ...
                  @submit="
                    ...
                    if(statusSelectedX !== statusSelected && statusSelected === 'publicated') {
                        if(!confirm('Opravdu si přejete nastavit projekt do stavu `{{ \App\Models\Project::getSTATUSES()['publicated']['title'] }}`?\nNa investory bude odeslána informace o spuštění projektu')) {
                            $event.preventDefault();
                            return;
                        }
                    }
                ">
                <input type="hidden" name="fileData" :value="JSON.stringify(tempFiles.fileList)">

<template x-for="(language, languageIndex) in languages" :key="languageIndex">
    <div x-cloak x-show="selectedLanguage === languageIndex">
        <div class="grid grid-cols-[200px_1fr] gap-x-3">
            <div>
                <template x-for="(languageSub, languageSubIndex) in language.sub" :key="languageSubIndex">
                    <div
                        ...
                        :class="{'bg-gray-500 text-white shadow': selectedLanguageCategory[selectedTab][languageIndex] === languageSubIndex}"
                    >
            ...
            <div>
                <template x-for="(languageSub, languageSubIndex) in language.sub" :key="languageSubIndex">
                    <div x-cloak
                         x-show="selectedLanguageCategory[selectedTab][languageIndex] === languageSubIndex"
                         x-data="{selectedTranslate: null}">

                        <template x-if="translateData[languageIndex]">
                            <template
                                x-for="(translate, translateIndex) in translateData[languageIndex][selectedLanguageCategory[selectedTab][languageIndex]]"
                                :key="translateIndex">
                                <div
                                        ...
                                        @click.prevent.stop="
                                                         () => {
                                                            if (
                                                                ...
                                                                && translateOriginData[languageIndex][selectedLanguageCategory[selectedTab][languageIndex]][selectedTranslate] !== translateData[languageIndex][selectedLanguageCategory[selectedTab][languageIndex]][selectedTranslate]
                                                            ) {
                                                                if (confirm('Zahodit změny?')) {
                                                                    translateData[languageIndex][selectedLanguageCategory[selectedTab][languageIndex]][selectedTranslate] = translateOriginData[languageIndex][selectedLanguageCategory[selectedTab][languageIndex]][selectedTranslate]
                                                                } else {
                                                                    return;
                                                                }
                                                            }
                                ...
                                >
                                    <div
                                        ...
                                        :class="{
                                                        'bg-red-600 px-1 text-white rounded-[3px]': (translateData[languageIndex][selectedLanguageCategory[selectedTab][languageIndex]][translateIndex] ?? '').trim() === '',
                                                        'opacity-30': (translateData[fromLanguage] ?
                                                                        (translateData[fromLanguage][selectedLanguageCategory[selectedTab][languageIndex]] ?
                                                                            (translateData[fromLanguage][selectedLanguageCategory[selectedTab][languageIndex]][translateIndex] ?
                                                                                translateData[fromLanguage][selectedLanguageCategory[selectedTab][languageIndex]][translateIndex] : ''
                                                                            ) : ''
                                                                        ) : ''
                                        ...
                                        <span
                                            class="fa-solid fa-circle-info text-[15px] text-blue-600 cursor-pointer group absolute top-[5px] right-[5px]">
                                                            ...
                                                            >
                                                                <div
                                                                    x-text="(selectedLanguageCategory[selectedTab][languageIndex] !== '__default__' ? selectedLanguageCategory[selectedTab][languageIndex] + '.' : '') + translateIndex"
                                                                    class="break-all font-Spartan-SemiBold leading-5"></div>
                                                                <div
                                                                    class="my-1 mb-2 border-b border-gray-500 border-dashed"></div>
                                                                <template
                                                                    x-for="(metaData, metaIndex) in languagesMeta[(selectedLanguageCategory[selectedTab][languageIndex] !== '__default__' ? selectedLanguageCategory[selectedTab][languageIndex] + '.' : '') + translateIndex]">
                                                                    <div>
                                    ...
                                    <div>
                                        <div
                                            x-html="(translateData[languageIndex][selectedLanguageCategory[selectedTab][languageIndex]][translateIndex] ?? '').replace(/&amp;nbsp;/g, '&amp;amp;nbsp;')"
                                        ...
                                        @if(!env('LANG_ADMIN_READONLY', true))
                                            <template x-if="selectedTranslate === translateIndex">
                                                <div
                                                    x-data="{
                                                                saveAction: false,
                                                                translateDataInputX: (translateData[languageIndex][selectedLanguageCategory[selectedTab][languageIndex]][translateIndex] ?? ''),
                                                                replaceHtml() {
                                                                ...
                                                                setValue(value) {
                                                                    translateData[languageIndex][selectedLanguageCategory[selectedTab][languageIndex]][translateIndex] = value.replace(/&nbsp;/g, ' ').replace(/\s+/g, ' ').replace(/ ,/g, ',').replace(/ \./g, '.');
                                                                },
                                                                clearChanges() {
                                                                    ...
                                                                    this.translateDataInputX = translateOriginData[languageIndex][selectedLanguageCategory[selectedTab][languageIndex]][translateIndex]
                                                                    translateData[languageIndex][selectedLanguageCategory[selectedTab][languageIndex]][translateIndex] = this.translateDataInputX

                                                                    this.$nextTick(() => {
                                                                        translateDivElement = document.getElementById('lng-translate-' + languageIndex + '-' + selectedLanguageCategory[selectedTab][languageIndex] + '-' + selectedTranslate)
                                                                ...
                                                                save(unselectTranslate = true) {
                                                                    if((translateData[languageIndex][selectedLanguageCategory[selectedTab][languageIndex]][translateIndex] ?? '') === translateOriginData[languageIndex][selectedLanguageCategory[selectedTab][languageIndex]][translateIndex]) {
                                                                        return;
                                                                    }
                                                                    saveData(translateIndex, translateData[languageIndex][selectedLanguageCategory[selectedTab][languageIndex]][translateIndex], selectedTab);
                                                                ...
                                                                checkChangeTranslateIndex() {
                                                                    if (translateOriginData[languageIndex][selectedLanguageCategory[selectedTab][languageIndex]][selectedTranslate] !== translateData[languageIndex][selectedLanguageCategory[selectedTab][languageIndex]][selectedTranslate]) {
                                                                        if(confirm('Zahodit změny?')) {
                                                                            translateData[languageIndex][selectedLanguageCategory[selectedTab][languageIndex]][selectedTranslate] = translateOriginData[languageIndex][selectedLanguageCategory[selectedTab][languageIndex]][selectedTranslate];
                                                                        } else {
                                                                            return false;
    ...
    {{----}}
                                                        ...
                                                        @keydown="
                                                                    ...
                                                                    "
                                                        x-html="replaceHtml()"
                                                        :id="'lng-translate-' + languageIndex + '-' + selectedLanguageCategory[selectedTab][languageIndex] + '-' + translateIndex"
                                                    >
                                                    </div>
                                                    ...
                                                    <button
                                                        ...
                                                        <i class="fa-solid fa-xmark text-gray-400/75 p-2 mt-0.5 hover:bg-gray-300 rounded"
                                                           :class="{'text-red-700': translateData[languageIndex][selectedLanguageCategory[selectedTab][languageIndex]][translateIndex] !== translateOriginData[languageIndex][selectedLanguageCategory[selectedTab][languageIndex]][translateIndex]}"></i>
                                                    </button>
                                                    <button
                                                        @click.prevent.stop="save(false)">
                                                        <i class="fa-solid fa-check text-gray-400/75 p-2 mt-0.5 hover:bg-gray-300 rounded ml-0.25"
                                                           :class="{'text-green-700': translateData[languageIndex][selectedLanguageCategory[selectedTab][languageIndex]][translateIndex] !== translateOriginData[languageIndex][selectedLanguageCategory[selectedTab][languageIndex]][translateIndex]}"
                                                        ></i>
                                                    </button>
                                                </div>
                                            </template>

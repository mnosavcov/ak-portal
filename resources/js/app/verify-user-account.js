...
Alpine.data('verifyUserAccount', (id) => ({
    ...
    nextBtnClick() {
        ...
        if (this.data.check_status === 'not_verified' && this.step < 4) {
            this.scrollToAnchor();
            this.step++;
            return;
        }

        if (this.data.check_status !== 'not_verified' && this.step < 2) {
            this.scrollToAnchor();
            this.step++;
            return;
        }
    ...
    nextBtnText() {
        if (this.data.check_status !== 'not_verified' && this.step > 1) {
            return 'Aktualizovat';
        }
